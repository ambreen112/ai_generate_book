...
* `τ` represents actuator joint torques.
* `Jᵀ(q) F_ext` represents forces from external contacts.
...
2.  **Dynamics Equation Components (10 points)**: Explain each term in the generalized robot dynamics equation: `M(q) q̈ + C(q, q̇) q̇ + G(q) = τ + Jᵀ(q) F_ext`onent and how it contributes to the robot's overall motion.
