...
Reinforcement learning involves an agent interacting with an environment over a sequence of time steps. At each step, the agent observes the current state ($s_t$), takes an action ($a_t$), and receives a reward ($r_t$) from the environment. The action transitions the environment to a new state ($s_{t+1}$). The agent's goal is to learn a policy $\pi(a|s)$ that maximizes the expected cumulative reward over time.
...
*   **State ($s$)**: A complete description of the environment at a given time (e.g., joint angles, sensor readings, object positions).
*   **Action ($a$)**: A decision made by the agent that affects the environment (e.g., motor commands, gripper movements).
*   **Reward ($r$)**: A scalar feedback signal from the environment, indicating the desirability of the agent's action. The design of effective reward functions is critical.
*   **Policy ($\pi$)**: A strategy that maps states to actions, dictating the agent's behavior.
